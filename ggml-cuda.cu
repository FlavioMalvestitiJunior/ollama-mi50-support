...
#else
            GGML_LOG_INFO("%s: skipping rocBLAS init (GGML_CUDA_FORCE_MMQ)\n", __func__);
#endif
        }
#endif
    ...
    void * alloc(size_t size, size_t * actual_size) override {
        ...
        if (allocate) {
            ...
            if (ggml_cuda_device_malloc(&ptr, look_ahead_size, device) != cudaSuccess) {
                    GGML_LOG_ERROR("hipMalloc FAILED: size=%zu MB, pool_size=%zu MB\n", look_ahead_size/1024/1024, pool_size/1024/1024);
...
// gfx906 fallback GEMM: computes dst = src0^T x src1
// gfx906 tiled GEMM: C[m,n] = sum_k A[m*lda+k] * B[n*ldb+k]; C is M×N col-major (ldc).
// Block (16,16): each block covers a 16×16 output tile.  BK=16 tiles the K dimension.
// ─── vega20 / gfx906 GEMM kernels ──────────────────────────────────────────────────────────
// Tile: 64×64 output per block, 4×4 sub-tile per thread (TM=TN=4).
// Block: dim3(16,16) = 256 threads — same as before, but each block now covers 16× more output.
// Each thread carries TM×TN = 16 float accumulators in registers — avoids repeated LDS writes.
// Per K-step: 4 A-elements + 4 B-elements loaded per thread; inner loop fully unrolled with FMA.
// LDS: sA[64][17] + sB[64][17] ≈ 8.7 KB — 7 blocks/CU fit within the 64 KB LDS budget.
// Arithmetic intensity: 64×64×2 / (2×64×4) = 8 FLOPs/byte  (vs 4 in the 16×16 version).
// +1 column padding on both tiles eliminates LDS bank conflicts on both loads and accumulate reads.

// Type conversion helper — specialised for half so no runtime branch is needed.
template<typename T>
struct vega20_f32 {
    __device__ __forceinline__ static float cvt(T v) { return float(v); }
};
template<>
struct vega20_f32<half> {
    __device__ __forceinline__ static float cvt(half v) { return __half2float(v); }
};

template<typename T0, typename T1>
static __global__ __launch_bounds__(256)
void vega20_gemm_tiled(
        const T0 * __restrict__ A, const T1 * __restrict__ B, float * __restrict__ C,
    ...
    const int tx   = threadIdx.x;            // [0, BM/TM) = [0,16)
    const int ty   = threadIdx.y;            // [0, BN/TN) = [0,16)
    const int flat = ty * (BM / TM) + tx;    // [0,256)

    // Shared tiles — +1 column padding avoids LDS bank conflicts.
    __shared__ float sA[BM][BK + 1];
    __shared__ float sB[BN][BK + 1];

    // Per-thread register accumulators (initialised to zero).
    float acc[TM][TN] = {};

    for (int k0 = 0; k0 < K; k0 += BK) {
        // Load sA[BM][BK]: BM*BK=1024 elements, 256 threads → 4 each (unrolled).
        #pragma unroll
        for (int i = 0; i < BM * BK / THREADS; i++) {
            const int idx = flat + i * THREADS;
            const int r = idx / BK, c = idx % BK;
            const int am = bm + r, ak = k0 + c;
            sA[r][c] = (am < M && ak < K) ? vega20_f32<T0>::cvt(A[am * lda + ak]) : 0.f;
        }
        // Load sB[BN][BK]: identical pattern.
        #pragma unroll
        for (int i = 0; i < BN * BK / THREADS; i++) {
            const int idx = flat + i * THREADS;
            const int r = idx / BK, c = idx % BK;
            const int br = bn + r, bc = k0 + c;
            sB[r][c] = (br < N && bc < K) ? vega20_f32<T1>::cvt(B[br * ldb + bc]) : 0.f;
        }
        __syncthreads();

        // Accumulate: load TM A-values and TN B-values into registers once per k,
        // then compute TM×TN FMAs — maximises register reuse, minimises LDS re-reads.
        #pragma unroll
        for (int k = 0; k < BK; k++) {
            float ra[TM], rb[TN];
            #pragma unroll
            for (int m = 0; m < TM; m++) ra[m] = sA[tx * TM + m][k];
            #pragma unroll
            for (int n = 0; n < TN; n++) rb[n] = sB[ty * TN + n][k];
            #pragma unroll
            for (int m = 0; m < TM; m++)
                #pragma unroll
                for (int n = 0; n < TN; n++)
                    acc[m][n] = __fmaf_rn(ra[m], rb[n], acc[m][n]);
        }
        __syncthreads();
    }

    // Write TM×TN output elements (column-major: C[col*ldc + row]).
    #pragma unroll
    for (int m = 0; m < TM; m++) {
        const int row = bm + tx * TM + m;
        if (row >= M) continue;
        #pragma unroll
        for (int n = 0; n < TN; n++) {
            const int col = bn + ty * TN + n;
            if (col < N) C[col * ldc + row] = acc[m][n];
        }
    }
}

// Batched variant — blockIdx.z selects the batch element; src0 broadcast via r2/r3.
template<typename T0, typename T1>
static __global__ __launch_bounds__(256)
void vega20_gemm_batched_tiled(
        const T0 * __restrict__ A_base, const T1 * __restrict__ B_base, float * __restrict__ C_base,
        int M, int N, int K, int lda, int ldb, int ldc,
        int64_t stride_a, int64_t stride_b, int64_t stride_c,
        int ne02, int ne12, int r2, int r3) {
    const int batch = blockIdx.z;
    const int i2    = batch % ne12, i3 = batch / ne12;
    const T0 * A = A_base + (int64_t)((i3 / r3) * ne02 + (i2 / r2)) * stride_a;
    const T1 * B = B_base + (int64_t)batch * stride_b;
    float     * C = C_base + (int64_t)batch * stride_c;
...
// GEMV for gfx906: y[M] = A[M×K] × x[K]  (single-token decode path, N=1)
// Block: dim3(32, ROWS) — one warp per row, ROWS rows per block (256 threads total).
// Each lane handles K/32 elements; warp_reduce_sum collapses to scalar per row.
// Memory: A loaded coalesced (32 consecutive K-elements per warp step); x reused from L2.
template<typename T0, typename T1, int ROWS = 8>
static __global__ __launch_bounds__(ROWS * 32)
void vega20_gemv(
        const T0 * __restrict__ A, const T1 * __restrict__ x, float * __restrict__ y,
        int M, int K, int lda) {
    const int row  = blockIdx.x * ROWS + (int)threadIdx.y;
    const int lane = (int)threadIdx.x;
    if (row >= M) return;

    float sum = 0.f;
    for (int k = lane; k < K; k += 32)
        sum = __fmaf_rn(vega20_f32<T0>::cvt(A[row * lda + k]),
                        vega20_f32<T1>::cvt(x[k]), sum);
...
// gfx906 (Vega20/MI50): batched non-quantized GEMM without rocBLAS
static void ggml_cuda_mul_mat_batched_vega20(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    GGML_TENSOR_BINARY_OP_LOCALS
    ...
    const int     K       = (int)ne10;
    const int     lda     = (int)(nb01 / ts_src0);           // src0 row stride (elements)
    const int     ldb     = (int)(nb11 / ts_src1);           // src1 row stride (elements)
    const int     ldc     = (int)ne0;
    const int64_t stride_a = nb02 / ts_src0;                 // src0 batch stride (elements)
    const int64_t stride_b = nb12 / ts_src1;                 // src1 batch stride (elements)
    const int64_t stride_c = (int64_t)ne0 * ne1;             // dst  batch stride (elements)
    const int     n_batch  = (int)(ne12 * ne13);

    const dim3 block(16, 16);                                          // (BM/TM, BN/TN)
    const dim3 grid((M + 63)/64, (N + 63)/64, n_batch);       // 64×64 tile; z = batch

    // Dispatch on (src0 type, src1 type) — both F16 and F32 handled natively in the kernel,
    // no intermediate F32 conversion needed.
    if (src0->type == GGML_TYPE_F16 && src1->type == GGML_TYPE_F16) {
        vega20_gemm_batched_tiled<half, half><<<grid, block, 0, stream>>>(
            (const half *)src0->data, (const half *)src1->data, (float *)dst->data,
            M, N, K, lda, ldb, ldc, stride_a, stride_b, stride_c,
            (int)ne02, (int)ne12, (int)r2, (int)r3);
    } else if (src0->type == GGML_TYPE_F16) {
        vega20_gemm_batched_tiled<half, float><<<grid, block, 0, stream>>>(
            (const half *)src0->data, (const float *)src1->data, (float *)dst->data,
            M, N, K, lda, ldb, ldc, stride_a, stride_b, stride_c,
            (int)ne02, (int)ne12, (int)r2, (int)r3);
    } else if (src1->type == GGML_TYPE_F16) {
        vega20_gemm_batched_tiled<float, half><<<grid, block, 0, stream>>>(
            (const float *)src0->data, (const half *)src1->data, (float *)dst->data,
            M, N, K, lda, ldb, ldc, stride_a, stride_b, stride_c,
            (int)ne02, (int)ne12, (int)r2, (int)r3);
    } else {
        vega20_gemm_batched_tiled<float, float><<<grid, block, 0, stream>>>(
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    } else if (!split && no_cublas && !ggml_is_quantized(src0->type)
        && !ggml_is_transposed(src0) && !ggml_is_transposed(src1) && src1->ne[2]*src1->ne[3] > 1) {
        // gfx906: batched non-quantized GEMM without rocBLAS
        ggml_cuda_mul_mat_batched_vega20(ctx, src0, src1, dst);
...
static enum ggml_status ggml_backend_cuda_graph_reserve(ggml_backend_t backend, ggml_cgraph * cgraph, bool alloc) {
    ...
    GGML_LOG_INFO("ggml_backend_cuda_graph_reserve: starting (alloc=%d, n_nodes=%d)\n", (int)alloc, cgraph->n_nodes);
    try {
        bool use_cuda_graph = false;
        bool cuda_graph_update_required = false;
        bool graph_evaluated_or_captured = false;
        ...
        GGML_LOG_ERROR("ggml_backend_cuda_graph_reserve: exception: %s\n", e.what());
        result = GGML_STATUS_FAILED;
    }
    GGML_LOG_INFO("ggml_backend_cuda_graph_reserve: done, result=%d\n", (int)result);
