...
// gfx906 fallback: sequential forward-substitution TRSM (A*X = B, A lower triangular, row-major)
// One block per batch, threads tile the k column dimension.
static __global__ void solve_tri_vega20(
        const float * __restrict__ A_base, const float * __restrict__ B_base, float * __restrict__ X_base,
        int n, int k,
        int64_t ne02, int64_t s02, int64_t s03, int64_t s12, int64_t s13, int64_t s2, int64_t s3) {
    const int64_t batch = blockIdx.x;
    const int64_t i3    = batch / ne02;
    const int64_t i2    = batch % ne02;

    const float * A = A_base + i3 * s03 + i2 * s02;
    const float * B = B_base + i3 * s13 + i2 * s12;
    float       * X = X_base + i3 * s3  + i2 * s2;

    for (int row = 0; row < n; row++) {
        for (int col = (int)threadIdx.x; col < k; col += (int)blockDim.x) {
            float val = B[row * k + col];
            for (int l = 0; l < row; l++) {
                val -= A[row * n + l] * X[l * k + col];
            }
            X[row * k + col] = val / A[row * n + row];
        }
        __syncthreads();
    }
}
...
void ggml_cuda_op_solve_tri(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    } else if (cc == GGML_CUDA_CC_VEGA20) {
        const int64_t total_batches = ne02 * ne03;
        const int     block_size    = (int)(k < 256 ? k : 256);
        solve_tri_vega20<<<(int)total_batches, block_size, 0, ctx.stream()>>>(
            (const float *) src0->data, (const float *) src1->data, (float *) dst->data,
            (int)n, (int)k, ne02,
            (int64_t)(src0->nb[2] / sizeof(float)), (int64_t)(src0->nb[3] / sizeof(float)),
            (int64_t)(src1->nb[2] / sizeof(float)), (int64_t)(src1->nb[3] / sizeof(float)),
            (int64_t)(dst->nb[2] / sizeof(float)),  (int64_t)(dst->nb[3] / sizeof(float)));
    } else {
